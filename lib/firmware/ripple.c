...
static void write_bytes(uint8_t **buf, size_t *len, uint8_t *bytes, size_t count)
{
                      ...
                      const uint8_t *pubkey, const uint8_t *sig)
{
    ...
    if (pubkey)
        write_vl(&buf, &len, &FM_signingPubkey, pubkey);
    if (sig)
        write_vl(&buf, &len, &FM_txnSignature, sig);
    if (source_address)
        write_address(&buf, &len, &FM_account, source_address);
    if (tx->payment.destination)
...
void ripple_signTx(const HDNode *node, RippleSignTx *tx,
    ...
    memset(resp->serialized, 0, sizeof(resp->serialized));

    // 'STX'
    strlcpy(resp->serialized, sizeof(resp->serialized), "\x53\x54\x58\x00");

    char source_address[MAX_ADDR_SIZE];
    if (!ripple_getAddress(node, source_address))
        return;

    uint8_t *buf = resp->serialized;
    size_t len = sizeof(resp->serialized) - 4;
    serialize(&buf, &len, source_address, node->public_key, NULL);

    // Ripple uses the first half of SHA512
    uint8_t hash[64];
    sha512_Raw(resp->serialized, buf - resp->serialized, hash);
    ...
    buf = resp->serialized;
    len = sizeof(resp->serialized);
    serialize(&buf, &len, source_address, node->public_key, resp->signature.bytes);
